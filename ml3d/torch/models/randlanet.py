...
class RandLANet(BaseModel):
    ...
    **Architecture**
    ...
    def __init__(
        ...
        self.decoder1 = SharedMLP(encoder_dim_list[-2] + dim_feature, encoder_dim_list[-2], transpose=True, activation_fn=nn.LeakyReLU(0.2))
        self.decoder2 = SharedMLP(encoder_dim_list[-3] + encoder_dim_list[-2], encoder_dim_list[-3], transpose=True, activation_fn=nn.LeakyReLU(0.2))
        self.decoder3 = SharedMLP(encoder_dim_list[-4] + encoder_dim_list[-3], encoder_dim_list[-4], transpose=True, activation_fn=nn.LeakyReLU(0.2))
        self.decoder4 = SharedMLP(encoder_dim_list[-5] + encoder_dim_list[-4], encoder_dim_list[-5], transpose=True, activation_fn=nn.LeakyReLU(0.2))

        dim_feature = encoder_dim_list[-5]
    ...
    def transform(self, data, attr, min_possibility_idx=None):
        ...
        inputs['labels'] = label
        inputs['img_geotransform'] = img_geotransform
        inputs['img_wkt'] = img_wkt
    ...
    def forward(self, inputs):
        ...
        feat_interpolation_1 = self.nearest_interpolation(feat, interpolation_indices_list[-1])
        feat_decoder_1 = torch.cat([encoder_feat_list[-2], feat_interpolation_1], dim=1)
        feat_decoder_1 = self.decoder1(feat_decoder_1)
        feat_interpolation_2 = self.nearest_interpolation(feat_decoder_1, interpolation_indices_list[-2])
        feat_decoder_2 = torch.cat([encoder_feat_list[-3], feat_interpolation_2], dim=1)
        feat_decoder_2 = self.decoder2(feat_decoder_2)
        feat_interpolation_3 = self.nearest_interpolation(feat_decoder_2, interpolation_indices_list[-3])
        feat_decoder_3 = torch.cat([encoder_feat_list[-4], feat_interpolation_3], dim=1)
        feat_decoder_3 = self.decoder3(feat_decoder_3)
        feat_interpolation_4 = self.nearest_interpolation(feat_decoder_3, interpolation_indices_list[-4])
        feat_decoder_4 = torch.cat([encoder_feat_list[-5], feat_interpolation_4], dim=1)
        feat_decoder_4 = self.decoder4(feat_decoder_4)
        feat = feat_decoder_4

        scores = self.fc1(feat)

        feat_pc = scores.squeeze(3).transpose(1, 2)
        pc_wh = inputs['xyz_ori'].to(self.device)
        feat_arr = torch.zeros(pc_wh.shape[0], 512, 512, feat_pc.shape[2]).to(self.device)
        for i in range(pc_wh.shape[0]):
            feat_arr[i] = feat_arr[i].index_put((pc_wh[i, :, 1], pc_wh[i, :, 0]), feat_pc[i])

        feat_arr = feat_arr.permute(0, 3, 1, 2)

        feat_arr = F.max_pool2d(feat_arr, kernel_size=7, stride=1, padding=3)

        # return scores.squeeze(3).transpose(1, 2)
        return feat_arr.permute(0,2,3,1)
        # return scores.squeeze(3).transpose(1, 2)

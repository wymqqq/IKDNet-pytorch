...
class SemSegMetric(object):
    ...
    def OA(self):
        oacc = np.diag(self.confusion_matrix).sum() / self.confusion_matrix.sum()
        return oacc
    ...
    def kappa(self):
        ...
        pe = np.multiply(sum0, sum1).sum()/(self.confusion_matrix.sum()**2)
        kappa = (po - pe) / (1 - pe)
        return kappa

    def F1_score(self):
        precision_cls = np.diag(self.confusion_matrix) / np.sum(self.confusion_matrix, axis=0)
        recall_cls = np.diag(self.confusion_matrix) / np.sum(self.confusion_matrix, axis=1)
        f1_scores = ((2 * precision_cls * recall_cls) / (precision_cls + recall_cls)).tolist()

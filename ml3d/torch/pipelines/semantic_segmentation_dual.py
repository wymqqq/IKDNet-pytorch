...
class SemanticSegmentationDual(BasePipeline):
    ...
    **Returns:**
    ...
    def run_test_img(self):
        ...
        with torch.no_grad():
            for unused_step, inputs in enumerate(tqdm(test_loader, desc='test')):
                ...
                labels_img = inputs['data']['masks']


                predict_scores, gt_labels = filter_valid_label(results[0], labels_img,
    ...
    def run_train(self):
        ...
        for epoch in range(1, cfg.max_epoch + 1):
            ...
            for step, inputs in enumerate(tqdm(train_loader, desc='training')):
                ...
                #     Loss, results, inputs['data']['masks'], device)
                loss, gt_labels, predict_scores = model.get_loss(
                    Loss, results, inputs['data']['masks'], inputs['data']['labels'], device)
    ...
    def save_logs(self, writer, epoch):
        ...
        # OA_dicts = [{
        #     'Training accuracy': OA,
        #     'Validation accuracy': val_OA
        # } for OA, val_OA in zip(train_OA, val_OA)]
        OA_dicts = {
            'Training OA': train_OA,
            'Validation OA': val_OA
        }
        ...
        log.info(f"OA train: {OA_dicts['Training OA']:.3f} "
                 f" eval: {OA_dicts['Validation OA']:.3f}")
...
PIPELINE._register_module(SemanticSegmentationDual, "torch")

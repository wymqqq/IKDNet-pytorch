...
class SemanticSegmentationImg(BasePipeline):
    ...
    **Returns:**
    ...
    def run_test(self):
        ...
        log_file_path = join(cfg.logs_dir, 'log_test.txt')
        shutil.copy(sys.modules[model.__module__].__file__, cfg.logs_dir)
        ...
        with torch.no_grad():
            for unused_step, inputs in enumerate(tqdm(test_loader, desc='test')):
                if hasattr(inputs['data'], 'to'):
                    inputs['data'].to(device)
                # results = model(inputs['data'])
                results = model(inputs['data'], inputs['attr'])
                predict_scores, gt_labels = filter_valid_label(results, inputs['data']['label'],
                                                               model.cfg.num_classes, model.cfg.ignored_label_inds, device)
                if predict_scores.size()[-1] != 0:
                    self.metric_test.update(predict_scores, gt_labels)

        log.info(f"IoU : {[float('{:.4f}'.format(i)) for i in self.metric_test.iou()]}")

        log.info(
            f"OA : {self.metric_test.OA():.4f}, mean Acc : {self.metric_test.acc()[-1]:.4f}, "
            f"kappa : {self.metric_test.kappa():.4f}, mIoU : {self.metric_test.iou()[-1]:.4f}"
        )

        log.info("Finished testing")

    def run_test_img(self):
        """Run the test using the data passed."""
        model = self.model
        dataset = self.dataset
        device = self.device
        ...
        with torch.no_grad():
            for unused_step, inputs in enumerate(tqdm(test_loader, desc='test')):
                ...
                predict_scores, gt_labels = filter_valid_label(results, labels,
                                                               model.cfg.num_classes, model.cfg.ignored_label_inds, device)
                if predict_scores.size()[-1] != 0:
                    self.metric_test.update(predict_scores, gt_labels)

        log.info(f"IoU : {[float('{:.4f}'.format(i)) for i in self.metric_test.iou()]}")
        log.info(f"F1 : {[float('{:.4f}'.format(i)) for i in self.metric_test.F1_score()]}")
        # log.info(
        #     f"Overall Testing Accuracy : {self.metric_test.acc()[-1]:.4f}, mIoU : {self.metric_test.iou()[-1]:.4f}"
        # )
        log.info(
            f"OA : {self.metric_test.OA():.4f}, mean Acc : {self.metric_test.acc()[-1]:.4f}, "
            f"kappa : {self.metric_test.kappa():.4f}, mIoU : {self.metric_test.iou()[-1]:.4f}"
    ...
    def run_train(self):
        ...
        log_file_path = join(cfg.logs_dir, 'log_train.txt')
        shutil.copy(sys.modules[model.__module__].__file__, cfg.logs_dir)
        shutil.copy(sys.modules[dataset.__module__].__file__, cfg.logs_dir)
        log.info("Logging in file : {}".format(log_file_path))
        log.addHandler(logging.FileHandler(log_file_path))
        ...
        train_loader = DataLoader(
            ...
            num_workers=cfg.get('num_workers', cfg.num_workers),
        ...
        for epoch in range(1, cfg.max_epoch + 1):
            ...
            for step, inputs in enumerate(tqdm(train_loader, desc='training')):
                ...
                #     print(inputs['attr']['name'])
                #     log.info(inputs['attr']['name'])
                ...
                # log.info(f"IoU : {[float('{:.4f}'.format(i)) for i in self.metric_train.iou()]}")
                # log.info(
                #     f"OA : {self.metric_train.OA():.4f}, mean Acc : {self.metric_train.acc()[-1]:.4f}, "
                #     f"kappa : {self.metric_train.kappa():.4f}, mIoU : {self.metric_train.iou()[-1]:.4f}"
                # )
            ...
            if self.metric_val.iou()[-1] > best_iou:
                best_iou = self.metric_val.iou()[-1]
                path_ckpt = join(self.cfg.logs_dir, 'checkpoint')
                make_dir(path_ckpt)
                file = glob.glob(str(Path(path_ckpt) / 'best_ckpt_iou*'))
                if file:
                    Path(file[0]).unlink(missing_ok=True)
                torch.save(
                    dict(epoch=epoch,
                         model_state_dict=self.model.state_dict(),
                         optimizer_state_dict=self.optimizer.state_dict(),
                         scheduler_state_dict=self.scheduler.state_dict()),
                    join(path_ckpt, f'best_ckpt_iou_{best_iou:4f}_{epoch:05d}.pth'))
                log.info(f'Save best ckpt best_ckpt_iou_{best_iou:4f}_{epoch:05d} to {path_ckpt:s}')

            if self.metric_val.acc()[-1] > best_aa:
                best_aa = self.metric_val.acc()[-1]
                path_ckpt = join(self.cfg.logs_dir, 'checkpoint')
                make_dir(path_ckpt)
                file = glob.glob(str(Path(path_ckpt) / 'best_ckpt_aa*'))
                if file:
                    Path(file[0]).unlink(missing_ok=True)
                torch.save(
                    dict(epoch=epoch,
                         ...
                         optimizer_state_dict=self.optimizer.state_dict(),
                         scheduler_state_dict=self.scheduler.state_dict()),
                    join(path_ckpt, f'best_ckpt_aa_{best_aa:4f}_{epoch:05d}.pth'))
                log.info(f'Save best ckpt best_ckpt_aa_{best_aa:4f}_{epoch:05d} to {path_ckpt:s}')

            if self.metric_val.OA() > best_oa:
                best_oa = self.metric_val.OA()
                path_ckpt = join(self.cfg.logs_dir, 'checkpoint')
                make_dir(path_ckpt)
                file = glob.glob(str(Path(path_ckpt) / 'best_ckpt_OA*'))
                if file:
                    Path(file[0]).unlink(missing_ok=True)
                torch.save(
                    dict(epoch=epoch,
                         ...
                         optimizer_state_dict=self.optimizer.state_dict(),
                         scheduler_state_dict=self.scheduler.state_dict()),
                    join(path_ckpt, f'best_ckpt_OA_{best_oa:4f}_{epoch:05d}.pth'))
                log.info(f'Save best ckpt best_ckpt_OA_{best_oa:4f}_{epoch:05d} to {path_ckpt:s}')
    ...
    def save_logs(self, writer, epoch):
        ...
        loss_dict = {
            ...
            'Training loss': np.nanmean(self.losses),
            'Validation loss': np.nanmean(self.valid_losses)
        }
        acc_dicts = [{
...
PIPELINE._register_module(SemanticSegmentationImg, "torch")

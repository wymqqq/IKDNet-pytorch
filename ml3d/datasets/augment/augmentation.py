...
class Augmentation():
    ...
    def std(self, pc, feat, cfg):
        """ standardization pointcloud and/or features.

        Points are normalized in [0, 1] and features can be standardizated.
        ...
        if 'points' in cfg:
            ...
            if cfg_p.get('method', 'linear') == 'linear':
                pc -= pc.mean(0)
                pc /= np.std(pc, axis=0).clip(min=1e-7)
            else:
                raise ValueError(f"Unsupported method : {cfg_p.get('method')}")
        ...
        if 'feat' in cfg and feat is not None:
            ...
            if cfg_f.get('method', 'linear') == 'linear':
                feat -= feat.mean(0)
                feat /= np.std(feat, axis=0).clip(min=1e-7)
            else:
                raise ValueError(f"Unsupported method : {cfg_f.get('method')}")
    ...
    def rotate(self, pc, cfg):
        ...
        # if np.abs(pc[:, :2].mean()) > 1e-2:       # 不适合SantaClara
        #     warnings.warn(
...
class SemsegAugmentation(Augmentation):
    ...
    def __init__(self, cfg, seed=None):
        ...
        all_methods = [
            'recenter', 'normalize', 'std', 'rotate', 'scale', 'noise',
    ...
    def augment(self, point, feat, labels, cfg, seed=None):
        ...
        if 'std' in cfg:
            point = self.std(point, feat, cfg['std'])
